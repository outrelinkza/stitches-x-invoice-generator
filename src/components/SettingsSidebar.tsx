...
export default function SettingsSidebar({ currentSection, onSectionChange }: SettingsSidebarProps) {
  ...
  return (
    <aside className="w-full lg:w-64 flex-shrink-0 glass-effect border-b lg:border-b-0 lg:border-r border-white/20 flex flex-row lg:flex-col p-4 animate-enter overflow-x-auto lg:overflow-x-visible" style={{animationDelay: '50ms'}}>
      <div className="flex items-center gap-2 px-1 mb-0 lg:mb-8 flex-shrink-0">
        <Link href="/" className="flex items-center gap-2">
          <svg className="text-white h-6 w-6 lg:h-8 lg:w-8" fill="none" viewBox="0 0 48 48" xmlns="http://www.w3.org/2000/svg">
            <path clipRule="evenodd" d="M24 4H6V17.3333V30.6667H24V44H42V30.6667V17.3333H24V4Z" fill="currentColor" fillRule="evenodd"></path>
          </svg>
          <span className="text-lg lg:text-xl font-bold text-white">InvoicePro</span>
        </Link>
      </div>

      <nav className="flex flex-row lg:flex-col gap-2 ml-4 lg:ml-0">
        {settingsCategories.map((category) => (
          <button 
            key={category.id}
            onClick={() => onSectionChange?.(category.id)}
            className={`flex items-center gap-2 lg:gap-3 px-2 lg:px-3 py-2 rounded-lg transition-colors text-left whitespace-nowrap flex-shrink-0 ${
          ...
          >
            ...
            <span className="text-xs lg:text-sm font-medium hidden sm:block">{category.label}</span>
          </button>
        ))}
      </nav>
