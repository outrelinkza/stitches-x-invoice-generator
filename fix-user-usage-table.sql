-- Fix user_usage table and RLS policies
...
CREATE POLICY "Authenticated users can view own usage" ON public.user_usage 
  FOR SELECT USING (
...
CREATE POLICY "Authenticated users can insert own usage" ON public.user_usage 
...
CREATE POLICY "Authenticated users can update own usage" ON public.user_usage 
  FOR UPDATE USING (
    auth.uid() IS NOT NULL AND (
      auth.uid() = user_id OR 
      (email IS NOT NULL AND email = auth.jwt() ->> 'email')
